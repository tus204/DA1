    ...
    if ($result) {
        if (password_verify($password, $result['password'])) {
            if ($result['verified'] == 0) {
                ...
                $sql = "UPDATE users SET verification_token = '$token', token_expires_at = '$expires_at' WHERE username = '$username'";
                pdo_execute($sql);

                $subject = 'Xác thực đăng ký tài khoản';
                $verification_link = "http://localhost/fpoly/du_an_1_nhom_8_newui/index.php?act=verify&token=$token";
                send_verification_email($result['email'], $verification_link, $subject);

                return 'Tài khoản của bạn chưa xác thực, vui lòng kiểm tra đường link trong email';
            }
...
function register_taikhoan($username, $email, $password)
{
    ...
    $subject = 'Xác thực đăng ký tài khoản';
    $verification_link = "http://localhost/fpoly/du_an_1_nhom_8_newui/index.php?act=verify&token=$token";
    send_verification_email($email, $verification_link, $subject);
}
...
function send_password_reset_link($email)
{
    ...
    $reset_link = "http://localhost/fpoly/du_an_1_nhom_8_newui/index.php?act=reset_password&token=$token";
    send_verification_email($email, $reset_link, $subject);
}
...
function add_taikhoan_admin($username, $password, $status)
{
    $hashed_password = password_hash($password, PASSWORD_DEFAULT);
    $sql = "INSERT INTO users (username, password, role, verified, status) VALUES ('$username', '$hashed_password', 0, 1, '$status')";
    pdo_execute($sql);
}
